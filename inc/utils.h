...
class JVector {
 ...
 public:
  JVector() : begin_(nullptr), len_(0), cap_(0) {}

  explicit JVector(size_t len) {
    this->cap_ = std::max(static_cast<size_t>(2), len);
    this->begin_ = new T[this->cap_];
    this->len_ = len;
  }
  JVector(const JVector& arr);
  JVector(JVector&& arr) noexcept;
  JVector& operator=(const JVector& arr);
  JVector& operator=(JVector&& arr) noexcept;
  ~JVector();
...
template <typename T>
JVector<T>::JVector(const JVector<T>& v) {
  this->cap_ = v.cap_;
  this->len_ = v.len_;
  this->begin_ = new T[this->cap_];
...
template <typename T>
JVector<T>::JVector(JVector<T>&& v) noexcept : begin_(v.begin_), len_(v.len_), cap_(v.cap_) {
  v.begin_ = nullptr;
  v.len_ = 0;
  v.cap_ = 0;
}
...
template <typename T>
JVector<T>& JVector<T>::operator=(const JVector<T>& v) {
  if (this != &v) {
...
template <typename T>
JVector<T>& JVector<T>::operator=(JVector<T>&& v) noexcept {
  if (this->begin_ != nullptr) delete[] this->begin_;
...
template <typename T>
T& JVector<T>::operator[](size_t index) {
  JST_DEBUG(index >= 0 && index < len_);
  return this->begin_[index];
}

template <typename T>
const T& JVector<T>::operator[](size_t index) const {
...
template <typename T>
size_t JVector<T>::find(const T& jn) const {
...
template <typename T>
size_t JVector<T>::insert(size_t pos, T& jn) {
  JST_DEBUG(pos <= size());
  if (this->cap_ == 0) {
    this->cap_ = 2;
    this->begin_ = new T[this->cap_];
...
template <typename T>
size_t JVector<T>::erase(size_t pos, size_t count) {
  JST_DEBUG(pos < size());
...
template <typename T>
void JVector<T>::push_back(const T& jn) {
  if (this->cap_ == 0) {
    this->cap_ = 2;
    this->begin_ = new T[this->cap_];
  }
...
template <typename T>
void JVector<T>::clear() {
  for (size_t i = 0; i < this->len_; i++) this->begin_[i].~T();
  this->len_ = 0;
}

template <typename T>
void JVector<T>::reserve(size_t new_cap) {
...
template <typename T>
bool operator==(const JVector<T>& v_1, const JVector<T>& v_2) {
